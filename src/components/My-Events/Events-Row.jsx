...
function EventsRow() {
  ...
  useEffect(() => {
    async function fetchBookedEvents() {
      ...
      if (error) {
        ...
        const eventsData = data.map(item => item.events).filter(e => e !== null);
  ...
  if (loading) {
    return <p className="text-xl p-4 flex items-center"><Loader2 className="animate-spin w-5 h-5 mr-2" /> Loading your events...</p>;
  }
  ...
  if (bookedEvents.length === 0) {
    return (
      <div className="p-4 flex flex-col justify-center">
        <p className="text-3xl font-bold ml-13 mb-4">upcoming events</p>
        <p className="text-gray-600">You currently have no upcoming events booked.</p>
      </div>
    );
  }
